...
static struct fuse_opt spfs_opts[] = {
	...
	SPFS_OPT("bitrate_preset=%d",		spotify_bitrate, -1),
	...
	switch(key) {
		case KEY_HELP:
			fprintf(stderr,
					...
					"		-o username=STRING\n"
					"		-o password=STRING\n"
					"		-o bitrate_preset=INT (0=default, 1=96kbps, 2=160kbps, 3=320kbps)\n"
					, outargs->argv[0]);
			exit(1);
		case KEY_VERSION:
...
static void load_configuration(struct spotifile_config *config)
{
	...
	if (config->spotify_bitrate < 0 ||
	    config->spotify_bitrate > 2) {
		config->spotify_bitrate = g_key_file_get_integer(config_file, "spotify", "bitrate_preset", &err);
	}
